...
{
    CLKGAT0 |= BIT(13); //SPI1 CLK enable

    //SPI1_G1: CLK_PB3, DO_PB4, DI PB5
    GPIOBFEN |= BIT(3) | BIT(4);
    GPIOBDE  |= BIT(3) | BIT(4);
    GPIOBDIR &= ~(BIT(3) | BIT(4)); //CLK,DO output
//    GPIOBDIR |= BIT(5);           //DI input
//    GPIOBPU  |= BIT(5);            //DI pull up 10K
    FUNCMCON1 |= (0x0F << 4);
    FUNCMCON1 |= (0x01 << 4);

    //SPI1_G2: CLK_PA6, DO_PA7, DI_PA5
//    GPIOAFEN |= BIT(6) | BIT(7);
//    GPIOADE  |= BIT(6) | BIT(7);
//    GPIOADIR &= ~(BIT(6) | BIT(7)); //CLK,DO output
//    GPIOADIR |= BIT(5);             //DI input
//    GPIOAPU   |= BIT(5);              //DI pull up 10K
//    FUNCMCON1 |= (0x0F << 4);
//    FUNCMCON1 |= (0x02 << 4);
...
//    GPIOBDIR |= BIT(0);             //DI input
//    GPIOBPU  |= BIT(0);              //DI pull up 10K
//    FUNCMCON1 |= (0x0F << 4);
//    FUNCMCON1 |= (0x03 << 4);

    SPI1BAUD = 24000000/baud - 1;   //sysclk/baud - 1
    SPI1CON = BIT(0);               //BIT(0) spi enable, master mode tx

    memset(rgb_buf, 0x11, sizeof(rgb_buf));
}
