...
void matrix_hw(int32_t* sum, int8_t* x0, int8_t *y0, s16 loop);                                         //for (int i = 0; i < loop1; i++) {sum += x0 * y0;}
void matrix_hw_1(int32_t* sum, int8_t* x0, int8_t *y0, s16 loop1, s16 loop2);                           //for (int i = 0; i < loop1; i++) {sum_temp = 0; for (int j = 0; j < loop2; j++) {sum_temp += x0[i * loop2 + j] * y0[j];}  sum[i] = sum_temp;}
...
ALWAYS_INLINE s32 clip(s32 x0, const s32 x1)
{
    s32 y0;
...
ALWAYS_INLINE u32 clipu(u32 x0, const u32 x1)
{
    u32 y0;
